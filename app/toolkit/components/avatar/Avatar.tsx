...
export function AvatarImage({ className = "", photo, name }: Props) {
  if (!photo) {
    return (
      <FaUserCircle
        className={`relative inline-block h-10 w-10 rounded-full ring-2 ring-white ${className}`}
      />
    );
  }
  return (
...
export function AvatarFull({ photo = "", title, subtitle }) {
  return (
    <div className="flex items-center">
      ...
      <div className="ml-3">
        <p className="m-0 text-sm font-medium text-gray-700 group-hover:text-gray-900">{title}</p>
